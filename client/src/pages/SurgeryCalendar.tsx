...
export default function SurgeryCalendar() {
  ...
  const catalogueSearchInput = useMemo(() => getCatalogueSearchInput(catalogueSearch), [catalogueSearch]);
  const catalogueQuery = trpc.operations.implantCatalogue.useQuery(catalogueSearchInput, { enabled: isAuthenticated && canWrite && Boolean(catalogueSearchInput) });
  ...
  const addImplant = trpc.operations.addSurgeryImplant.useMutation({ onSuccess: () => { setCatalogueId(""); setDirectName(""); setDirectManufacturer(""); setDirectCode(""); setQuantity("1"); setUnitPrice(""); setImplantNotes(""); refresh("Implant registered with its surgery price."); }, onError: (error) => setNotice(error.message) });
