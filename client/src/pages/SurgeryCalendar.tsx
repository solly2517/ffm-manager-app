...
const isSurgeryDayOrLater = (value: Date | string) => {
  const surgeryDate = new Date(value); surgeryDate.setHours(0, 0, 0, 0);
...
export default function SurgeryCalendar() {
  ...
  const [implantCurrency, setImplantCurrency] = useState("SAR");
  ...
  const previewAmount = Number(implantQuantity || 0) * Number(unitPrice || 0);

  const surgeriesByDay = useMemo(() => {
    const byDay = new Map<string, typeof calendarQuery.data>();
    for (const surgery of calendarQuery.data ?? []) {
      const key = dateKey(surgery.surgeryDate);
      byDay.set(key, [...(byDay.get(key) ?? []), surgery]);
    }
    return byDay;
  }, [calendarQuery.data]);
  const days = useMemo(() => {
  ...
  const addImplant = trpc.operations.addSurgeryImplant.useMutation({ onSuccess: () => { setCatalogueId(""); setImplantQuantity("1"); setUnitPrice(""); setImplantCurrency("SAR"); setLotNumber(""); setSerialNumber(""); setImplantNotes(""); refreshSurgery("Priced approved implant registered."); }, onError: (error) => setNotice(error.message) });
  ...
  useEffect(() => {
    ...
    setLifecycleAction("completed"); setLifecycleReason(""); setRescheduledDate(""); setNotice("");
  }, [selected?.id]);

  const uploadPatientSheet = (file?: File) => {
    if (!file || !selected) return;
    const accepted = ["application/pdf", "image/jpeg", "image/png", "image/webp"] as const;
    if (!(accepted as readonly string[]).includes(file.type)) { setNotice("Upload a PDF, JPEG, PNG, or WEBP patient sheet."); return; }
  ...
  return <div className="manager-shell">
    <aside className="manager-sidebar"><div className="brand-lockup"><div className="logo-mark small">FFM</div><div><strong>FFM Calendar</strong><span>Surgery planning</span></div></div><div className="sidebar-rule"/><nav className="space-y-1"><a className="sidebar-link" href="/"><ArrowLeft size={17}/><span>Back to workspace</span></a>{isAdmin && <a className="sidebar-link" href="/?workspace=surgeries"><CalendarDays size={17}/><span>Create surgery</span></a>}</nav><div className="sidebar-user"><div className="avatar">{(user?.name || user?.email || "F")[0].toUpperCase()}</div><div className="user-copy"><strong>{user?.name || user?.email || "Authenticated user"}</strong><span>{user?.role?.replace("_", " ")}</span></div><button className="logout-icon" onClick={() => logout()} title="Sign out">Sign out</button></div></aside>
    <main className="manager-main"><header className="manager-topbar"><div><p className="topbar-kicker">FFM / SHARED PLANNING</p><h2>Surgery calendar</h2></div><div className="live-indicator"><span/> All roles can view</div></header>
      <section className="manager-content"><div className="page-intro"><div><p className="eyebrow">Day-of-surgery control</p><h1>Plan early. Resolve only on the surgery day.</h1><p className="muted">Authorised users register approved implants with their actual unit price and country currency before closing a surgery.</p></div><Badge variant="outline">{calendarQuery.data?.length ?? 0} live surgeries</Badge></div>
      {notice && <div className={notice.includes("updated") || notice.includes("registered") || notice.includes("uploaded") || notice.includes("added") ? "admin-feedback success" : "admin-feedback error"}>{notice}</div>}
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(340px,0.9fr)]">
        <Card className="blueprint-card"><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>{month.toLocaleDateString(undefined, { month: "long", year: "numeric" })}</CardTitle><p className="muted">Select a surgery to manage its controlled lifecycle.</p></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setMonth((current) => new Date(current.getFullYear(), current.getMonth() - 1, 1))}><ChevronLeft size={16}/></Button><Button size="sm" variant="outline" onClick={() => setMonth((current) => new Date(current.getFullYear(), current.getMonth() + 1, 1))}><ChevronRight size={16}/></Button></div></div></CardHeader><CardContent>{calendarQuery.isLoading ? <div className="admin-feedback">Loading the shared surgery schedule…</div> : calendarQuery.error ? <div className="admin-feedback error">Unable to load calendar: {calendarQuery.error.message}</div> : <><div className="grid grid-cols-7 gap-1 text-center text-xs font-medium uppercase tracking-wide text-slate-400">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <div key={day} className="py-2">{day}</div>)}</div><div className="grid grid-cols-7 gap-1">{days.map((day) => { const events = surgeriesByDay.get(dateKey(day)) ?? []; const inMonth = day.getMonth() === month.getMonth(); return <div key={day.toISOString()} className={`min-h-28 border border-blue-500/20 p-1.5 ${inMonth ? "bg-slate-950/25" : "bg-slate-950/60 text-slate-600"}`}><div className="mb-1 text-xs font-semibold">{day.getDate()}</div><div className="space-y-1">{events.slice(0, 2).map((surgery) => <button key={surgery.id} onClick={() => setSelectedSurgeryId(surgery.id)} className={`block w-full border px-1.5 py-1 text-left text-[10px] leading-tight ${statusClasses[surgery.calendarStatus]} ${selectedSurgeryId === surgery.id ? "ring-1 ring-white" : ""}`}><strong className="block truncate">{surgery.procedureName || "Surgery"}</strong><span className="block truncate">{surgery.hospital || "Hospital pending"}</span></button>)}{events.length > 2 && <span className="block text-[10px] text-slate-400">+{events.length - 2} more</span>}</div></div>; })}</div></>}</CardContent></Card>
        <Card className="blueprint-card"><CardHeader><CardTitle>{selected ? selected.procedureName || "Surgery record" : "Select a surgery"}</CardTitle><p className="muted">{selected ? `${selected.hospital || "Hospital pending"} · ${new Date(selected.surgeryDate).toLocaleString()}` : "Choose any calendar entry to view its clinical operation record."}</p></CardHeader><CardContent>{!selected ? <div className="admin-feedback">Select a surgery in the calendar to review its appointment and requirements.</div> : <div className="space-y-5">
          <div className="rounded-md border border-blue-500/25 bg-slate-950/35 p-3 text-sm"><div className="flex flex-wrap gap-2"><Badge className={statusClasses[selected.calendarStatus]}>{selected.calendarStatus}</Badge><Badge variant="outline">Notified {new Date(selected.notifiedAt).toLocaleDateString()}</Badge></div>{selected.lifecycleReason && <p className="mt-3 text-slate-300"><strong>Lifecycle note:</strong> {selected.lifecycleReason}</p>}</div>
          {canWrite && !isFinal && <div className="space-y-3 border-t border-blue-500/20 pt-4"><Label>Appointment details</Label><Input type="datetime-local" value={scheduleDate} onChange={(event) => setScheduleDate(event.target.value)}/><select className="blueprint-input w-full" value={scheduleStatus} onChange={(event) => setScheduleStatus(event.target.value as ScheduleStatus)}><option value="notified">Notified</option><option value="confirmed">Confirmed</option></select><Button className="blueprint-button w-full" disabled={!scheduleDate || updateSchedule.isPending} onClick={() => updateSchedule.mutate({ id: selected.id, surgeryDate: new Date(scheduleDate), calendarStatus: scheduleStatus })}>{updateSchedule.isPending ? "Saving appointment…" : "Save appointment"}</Button></div>}
          {canResolveToday ? <div className="space-y-3 border-t border-blue-500/20 pt-4"><div className="flex items-center gap-2"><CheckCircle2 size={17}/><strong>Day-of-surgery outcome</strong></div><select className="blueprint-input w-full" value={lifecycleAction} onChange={(event) => setLifecycleAction(event.target.value as LifecycleAction)}><option value="completed">C — Surgery done</option><option value="postponed">A — Postponed to a new date</option><option value="cancelled">B — Cancelled</option></select>{lifecycleAction === "postponed" && <Input type="datetime-local" value={rescheduledDate} onChange={(event) => setRescheduledDate(event.target.value)}/>}<Input value={lifecycleReason} onChange={(event) => setLifecycleReason(event.target.value)} placeholder={lifecycleAction === "completed" ? "Completion note (optional)" : "Reason is required"}/>{lifecycleAction === "completed" && <p className="text-xs text-slate-400">Completion requires at least one priced approved implant and one patient-sheet proof. Current: {resourcesQuery.data?.implants.length ?? 0} implant(s), {resourcesQuery.data?.deliveryProofs.length ?? 0} proof(s).</p>}<Button className="blueprint-button w-full" disabled={resolveLifecycle.isPending || (lifecycleAction !== "completed" && !lifecycleReason.trim()) || (lifecycleAction === "postponed" && !rescheduledDate) || (lifecycleAction === "completed" && !completionReady)} onClick={() => resolveLifecycle.mutate({ id: selected.id, action: lifecycleAction, reason: lifecycleReason || undefined, rescheduledDate: lifecycleAction === "postponed" && rescheduledDate ? new Date(rescheduledDate) : undefined })}>{resolveLifecycle.isPending ? "Saving outcome…" : lifecycleAction === "completed" ? "Mark surgery completed" : lifecycleAction === "postponed" ? "Postpone surgery" : "Cancel surgery"}</Button></div> : canWrite && !isFinal ? <div className="admin-feedback">Day-of-surgery actions will be available on the scheduled surgery date.</div> : null}
          <div className="border-t border-blue-500/20 pt-4"><div className="mb-3 flex items-center gap-2"><PackagePlus size={17}/><strong>Approved implants used</strong></div>{resourcesQuery.isLoading ? <p className="muted">Loading materials…</p> : resourcesQuery.data?.implants.length ? <div className="space-y-2">{resourcesQuery.data.implants.map((implant) => <div className="rounded-md border border-blue-500/20 p-2 text-sm" key={implant.id}><div className="flex items-start justify-between gap-3"><strong>{implant.catalogueName || implant.implantName} × {implant.quantity}</strong><strong className="text-emerald-200">{formatMoney(implant.lineTotal, implant.currency)}</strong></div><span className="block text-xs text-slate-400">{formatMoney(implant.unitPrice, implant.currency)} each · {implant.catalogueManufacturer || "Approved catalogue"}{implant.catalogueProductCode ? ` · ${implant.catalogueProductCode}` : ""}{implant.lotNumber ? ` · Lot ${implant.lotNumber}` : ""}{implant.serialNumber ? ` · Serial ${implant.serialNumber}` : ""}</span></div>)}</div> : <p className="muted">No approved implants registered yet.</p>}{resourcesQuery.data?.implantTotals.length ? <div className="mt-3 rounded-md border border-emerald-400/30 bg-emerald-500/10 p-3 text-sm"><strong>Implant total</strong>{resourcesQuery.data.implantTotals.map((total) => <span className="block text-lg text-emerald-100" key={total.currency}>{formatMoney(total.total, total.currency)}</span>)}</div> : null}{canWrite && !isFinal && <div className="mt-3 space-y-2"><Label>Select directly from the approved catalogue</Label><select className="blueprint-input w-full" value={catalogueId} onChange={(event) => setCatalogueId(event.target.value)}><option value="">Choose approved implant</option>{(catalogueQuery.data ?? []).map((item) => <option key={item.id} value={item.id}>{item.name}{item.manufacturer ? ` — ${item.manufacturer}` : ""}{item.productCode ? ` (${item.productCode})` : ""}</option>)}</select>{!catalogueQuery.data?.length && <p className="text-xs text-amber-200">An Administrator must add an approved implant to the catalogue first.</p>}<div className="grid grid-cols-2 gap-2"><Input type="number" min="1" value={implantQuantity} onChange={(event) => setImplantQuantity(event.target.value)} placeholder="Quantity"/><Input type="number" min="0.01" step="0.01" value={unitPrice} onChange={(event) => setUnitPrice(event.target.value)} placeholder="Unit price"/></div><div className="grid grid-cols-2 gap-2"><Input value={implantCurrency} maxLength={3} onChange={(event) => setImplantCurrency(event.target.value.toUpperCase())} placeholder="Currency (SAR)"/><div className="rounded-md border border-blue-500/25 px-3 py-2 text-sm text-slate-200">Line amount: <strong>{formatMoney(Number.isFinite(previewAmount) ? previewAmount : 0, implantCurrency || "SAR")}</strong></div></div><div className="grid grid-cols-2 gap-2"><Input value={lotNumber} onChange={(event) => setLotNumber(event.target.value)} placeholder="Lot"/><Input value={serialNumber} onChange={(event) => setSerialNumber(event.target.value)} placeholder="Serial"/></div><Input value={implantNotes} onChange={(event) => setImplantNotes(event.target.value)} placeholder="Optional implant note"/><Button className="blueprint-button w-full" disabled={!catalogueId || !Number(implantQuantity) || Number(unitPrice) <= 0 || implantCurrency.trim().length !== 3 || addImplant.isPending} onClick={() => addImplant.mutate({ surgeryId: selected.id, implantCatalogueId: Number(catalogueId), quantity: Number(implantQuantity), unitPrice: Number(unitPrice), currency: implantCurrency.trim(), lotNumber: lotNumber || undefined, serialNumber: serialNumber || undefined, notes: implantNotes || undefined })}><PackagePlus size={16}/>{addImplant.isPending ? "Registering…" : "Register priced implant"}</Button></div>}</div>
          <div className="border-t border-blue-500/20 pt-4"><div className="mb-3 flex items-center gap-2"><FileCheck2 size={17}/><strong>Patient-sheet delivery proof</strong></div>{resourcesQuery.data?.deliveryProofs.length ? <div className="space-y-2">{resourcesQuery.data.deliveryProofs.map((proof) => <a key={proof.id} className="block rounded-md border border-blue-500/20 p-2 text-sm hover:border-blue-300" href={proof.url} target="_blank" rel="noreferrer"><strong className="block truncate">{proof.originalName}</strong><span className="block text-xs text-slate-400">{new Date(proof.createdAt).toLocaleString()} · {(proof.sizeBytes / 1024 / 1024).toFixed(2)} MB</span>{proof.note && <span className="mt-1 block text-slate-300">{proof.note}</span>}</a>)}</div> : <p className="muted">No patient-sheet proof uploaded yet.</p>}{canWrite && !isFinal && <div className="mt-3 space-y-2"><Input value={proofNote} onChange={(event) => setProofNote(event.target.value)} placeholder="Hospital handover note (optional)"/><Label className="flex cursor-pointer items-center justify-center gap-2 border border-blue-400/40 bg-blue-500/10 px-3 py-2 text-sm text-blue-100"><Upload size={16}/> Upload patient sheet<input className="hidden" type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(event) => uploadPatientSheet(event.target.files?.[0])}/></Label>{uploadProof.isPending && <p className="muted">Uploading hospital proof…</p>}</div>}</div>
          {isAdmin && <details className="border-t border-blue-500/20 pt-4"><summary className="cursor-pointer text-sm font-medium text-blue-100">Manage approved implant catalogue</summary><div className="mt-3 space-y-2"><Input value={catalogueName} onChange={(event) => setCatalogueName(event.target.value)} placeholder="Implant name"/><Input value={catalogueManufacturer} onChange={(event) => setCatalogueManufacturer(event.target.value)} placeholder="Manufacturer (optional)"/><Input value={catalogueCode} onChange={(event) => setCatalogueCode(event.target.value)} placeholder="Product code (optional)"/><Button className="blueprint-button w-full" disabled={!catalogueName.trim() || addCatalogue.isPending} onClick={() => addCatalogue.mutate({ name: catalogueName.trim(), manufacturer: catalogueManufacturer || undefined, productCode: catalogueCode || undefined })}>{addCatalogue.isPending ? "Adding implant…" : "Add approved implant"}</Button></div></details>}
