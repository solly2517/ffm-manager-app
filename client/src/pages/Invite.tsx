...
export default function Invite() {
  ...
  if (!isAuthenticated) return <div className="blueprint-page login-view"><Card className="login-card blueprint-card"><div className="logo-mark">FFM</div><p className="eyebrow">SECURE INVITATION</p><h1>Join FFM</h1><p className="muted">This secure link activates <strong>{preview.data.email}</strong> as a <strong>{preview.data.role}</strong> in FFM. No separate WebDev access is required.</p><p className="muted">You do not need to wait for an email code. Select the button below from this invitation link; the FFM invitation remains valid until {new Date(preview.data.expiresAt).toLocaleString()}.</p><Button className="w-full mt-6 blueprint-button" disabled={acceptMagic.isPending} onClick={() => acceptMagic.mutate({ token })}>{acceptMagic.isPending ? "Activating secure access…" : "Activate FFM access"}</Button>{acceptMagic.error && <div className="admin-feedback error">{acceptMagic.error.message}</div>}</Card></div>;
