...
export default function Invite() {
  ...
  if (!isAuthenticated) return <div className="blueprint-page login-view"><Card className="login-card blueprint-card"><div className="logo-mark">FFM</div><p className="eyebrow">SECURE INVITATION</p><h1>Join FFM</h1><p className="muted">This invitation is for <strong>{preview.data.email}</strong> as a {preview.data.role}. Sign in with that email to continue.</p><Button className="w-full mt-6 blueprint-button" onClick={() => startLogin()}>Sign in to accept</Button></Card></div>;
