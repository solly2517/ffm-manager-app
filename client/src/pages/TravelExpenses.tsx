...
export default function TravelExpenses() {
  ...
  const accountingRangeExport = trpc.travelExpenses.accountingExport.useQuery({ from: accountingFrom, to: accountingTo }, { enabled: false });
  ...
  const downloadRangeAccounting = async (format: "xlsx" | "csv") => {
    ...
    const rangeLabel = `${accountingFrom}_to_${accountingTo}`;
  ...
  return <div className="manager-shell">
    ...
    <main className="manager-main">
      ...
      <section className="manager-content space-y-6">
        ...
        {isAccountingExporter && <Card className="blueprint-card"><CardHeader><CardTitle className="flex items-center gap-2"><FileSpreadsheet size={20} /> Accounting export by date range</CardTitle><p className="muted">Choose any inclusive claim-date period. Excel includes separate claims and expense-line sheets; CSV contains the claim summary.</p></CardHeader><CardContent className="flex flex-wrap items-end gap-3"><div className="space-y-2"><Label htmlFor="accounting-from">From</Label><Input id="accounting-from" type="date" value={accountingFrom} onChange={event => setAccountingFrom(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="accounting-to">To</Label><Input id="accounting-to" type="date" min={accountingFrom || undefined} value={accountingTo} onChange={event => setAccountingTo(event.target.value)} /></div><Button className="blueprint-button" disabled={accountingRangeExport.isFetching} onClick={() => downloadRangeAccounting("xlsx")}><FileSpreadsheet size={16} /> {accountingRangeExport.isFetching ? "Preparing…" : "Download Excel"}</Button><Button variant="outline" disabled={accountingRangeExport.isFetching} onClick={() => downloadRangeAccounting("csv")}><Download size={16} /> Download CSV</Button></CardContent></Card>}
