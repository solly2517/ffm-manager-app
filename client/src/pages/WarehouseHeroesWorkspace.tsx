...
import { filterWarehouseDeliveryProofs } from "@/lib/warehouseProofs";
...
export function WarehouseHeroesWorkspace({ isAdmin, onBack }: WarehouseHeroesWorkspaceProps) {
  ...
  const proofsQuery = trpc.operations.warehouseDeliveryProofs.useQuery(undefined, { refetchInterval: 30_000 });
  ...
  const [proofHeroFilter, setProofHeroFilter] = useState("all");
  ...
  const proofHeroOptions = Array.from(new Map((proofsQuery.data ?? []).map((proof) => [proof.warehouseHeroId, proof])).values());
  const visibleProofs = filterWarehouseDeliveryProofs(proofsQuery.data ?? [], proofHeroFilter);
  ...
  return <div className="manager-shell">
    ...
    <main className="manager-main">
      ...
      <section className="manager-content">
        ...
        <Card className="blueprint-card section-card mt-4"><div className="section-heading"><div><p className="eyebrow">Delivery evidence</p><h2>Recent delivery proofs</h2><p className="muted">Photos uploaded by Warehouse Heroes assigned to this Manager.</p></div>{proofHeroOptions.length > 1 && <label className="flex flex-col gap-1 text-xs text-slate-300"><span>Filter by Warehouse Hero</span><select aria-label="Filter delivery proofs by Warehouse Hero" className="admin-role-select min-w-52" value={proofHeroFilter} onChange={(event) => setProofHeroFilter(event.target.value)}><option value="all">All assigned Heroes</option>{proofHeroOptions.map((proof) => <option key={proof.warehouseHeroId} value={proof.warehouseHeroId}>{proof.warehouseHeroName || proof.warehouseHeroEmail || `Warehouse Hero #${proof.warehouseHeroId}`}</option>)}</select></label>}</div>{proofsQuery.isLoading ? <div className="admin-feedback">Loading delivery proofs…</div> : proofsQuery.error ? <div className="admin-feedback error">{proofsQuery.error.message}</div> : proofsQuery.data?.length ? visibleProofs.length ? <div className="grid gap-3 md:grid-cols-2">{visibleProofs.map((proof) => <div className="activity-row" key={proof.id}><a href={proof.url} target="_blank" rel="noreferrer"><img src={proof.url} alt={`Delivery proof from ${proof.warehouseHeroName || "Warehouse Hero"}`} className="h-20 w-20 rounded-md object-cover"/></a><div><strong>{proof.warehouseHeroName || proof.warehouseHeroEmail || "Warehouse Hero"}</strong><span>{proof.note || "Delivery proof photo"}</span><span>{new Date(proof.capturedAt).toLocaleString()}</span></div></div>)}</div> : <div className="admin-feedback">No delivery proofs match the selected Warehouse Hero.</div> : <div className="admin-feedback">No delivery proof photos are available for the current assignment scope.</div>}</Card>
        {isAdmin && <Card className="blueprint-card section-card mt-4"><div className="section-heading"><div><p className="eyebrow">Administrator-only control</p><h2>Warehouse Hero onboarding and assignments</h2><p className="muted">Invite logistics personnel, then assign each Hero to the Manager responsible for their hospital deliveries.</p></div></div><div className="admin-add-row"><Input type="email" value={heroEmail} onChange={(event) => setHeroEmail(event.target.value)} placeholder="warehouse.hero@example.com"/><Button className="blueprint-button" disabled={!heroEmail || inviteHero.isPending} onClick={() => inviteHero.mutate({ email: heroEmail, role: "warehouse_hero" })}><UserPlus size={15}/>{inviteHero.isPending ? "Creating…" : "Invite Warehouse Hero"}</Button></div>{inviteHero.error && <div className="admin-feedback error">{inviteHero.error.message}</div>}{inviteUrl && <div className="invite-link-row"><Input value={inviteUrl} readOnly/><Button variant="outline" onClick={copyHeroInviteLink}>Copy link</Button>{copyNotice && <span className={copyNotice.startsWith("Invitation") ? "copy-notice success" : "copy-notice error"}>{copyNotice}</span>}</div>}<div className="inline-form mt-4"><select className="admin-role-select" value={managerId} onChange={(event) => setManagerId(event.target.value)}><option value="">Choose manager</option>{managersQuery.data?.map((manager) => <option key={manager.id} value={manager.id}>{manager.name || manager.email || `Manager #${manager.id}`}</option>)}</select><select className="admin-role-select" value={heroId} onChange={(event) => setHeroId(event.target.value)}><option value="">Choose Warehouse Hero</option>{adminHeroesQuery.data?.map((hero) => <option key={hero.id} value={hero.id}>{hero.name || hero.email || `Warehouse Hero #${hero.id}`}</option>)}</select><Button className="blueprint-button" disabled={!managerId || !heroId || assign.isPending} onClick={() => assign.mutate({ managerId: Number(managerId), warehouseHeroId: Number(heroId) })}><UserPlus size={15}/>{assign.isPending ? "Assigning…" : "Assign Hero"}</Button></div>{assign.error && <div className="admin-feedback error">{assign.error.message}</div>}{assignmentsQuery.isLoading ? <div className="admin-feedback">Loading Warehouse Hero assignments…</div> : assignmentsQuery.error ? <div className="admin-feedback error">{assignmentsQuery.error.message}</div> : assignmentsQuery.data?.length ? <div className="data-table"><div className="table-row table-head"><span>Manager</span><span>Warehouse Hero</span><span>Assigned</span><span>Action</span></div>{assignmentsQuery.data.map((assignment) => <div className="table-row" key={assignment.id}><span>{assignment.managerName}</span><span>{assignment.warehouseHeroName}</span><span>{new Date(assignment.createdAt).toLocaleDateString()}</span><span><Button variant="ghost" size="sm" disabled={unassign.isPending} onClick={() => unassign.mutate({ id: assignment.id })}>{unassign.isPending ? "Removing…" : "Unassign"}</Button></span></div>)}</div> : <div className="admin-feedback">No Manager–Warehouse Hero assignments exist yet.</div>}</Card>}
      </section>
    </main>
  </div>;
}
