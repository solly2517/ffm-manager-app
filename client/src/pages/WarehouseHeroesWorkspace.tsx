  ...
  return <div className="manager-shell">
    ...
    <main className="manager-main">
      ...
      <section className="manager-content">
        ...
        <Card className="blueprint-card section-card mt-4"><div className="section-heading"><div><p className="eyebrow">Delivery evidence</p><h2>Recent delivery proofs</h2><p className="muted">Photos uploaded by Warehouse Heroes assigned to this Manager.</p></div></div>{proofsQuery.isLoading ? <div className="admin-feedback">Loading delivery proofs…</div> : proofsQuery.error ? <div className="admin-feedback error">{proofsQuery.error.message}</div> : proofsQuery.data?.length ? <div className="grid gap-3 md:grid-cols-2">{proofsQuery.data.map((proof) => <div className="activity-row" key={proof.id}><a href={proof.url} target="_blank" rel="noreferrer"><img src={proof.url} alt={`Delivery proof from ${proof.warehouseHeroName || "Warehouse Hero"}`} className="h-20 w-20 rounded-md object-cover"/></a><div><strong>{proof.warehouseHeroName || proof.warehouseHeroEmail || "Warehouse Hero"}</strong><span>{proof.note || "Delivery proof photo"}</span><span>{new Date(proof.capturedAt).toLocaleString()}</span></div></div>)}</div> : <div className="admin-feedback">No delivery proof photos are available for the current assignment scope.</div>}</Card>
