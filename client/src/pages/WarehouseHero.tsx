...
export default function WarehouseHero() {
  ...
  if (!isAuthenticated) return <div className="blueprint-page login-view"><Card className="login-card blueprint-card"><div className="login-language"><LanguageSwitcher /></div><div className="logo-mark">FFM</div><p className="eyebrow">{t("warehouseHeroApp")}</p><h1>FFM Logistics</h1><p className="muted">{t("logisticsLoginDescription")}</p><Button className="w-full mt-6 blueprint-button" onClick={() => startLogin()}>{t("signIn")}</Button></Card></div>;
  if (!isWarehouseHero) return <div className="blueprint-page login-view"><Card className="login-card blueprint-card"><Truck size={32}/><h1>{t("warehouseHeroAccessRequired")}</h1><p className="muted">{t("warehouseHeroAccessDescription")}</p><Button variant="outline" onClick={() => logout()}><LogOut size={15}/> {t("signOut")}</Button></Card></div>;
