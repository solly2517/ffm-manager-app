...
export default function WarehouseHero() {
  ...
  const submitProofs = async () => {
    ...
    try {
      for (let index = 0; index < capturedProofs.length; index += 1) {
        await uploadProof.mutateAsync({ fileName: `live-camera-${Date.now()}-${index + 1}.jpg`, mimeType: "image/jpeg", captureSource: "live_camera", base64: capturedProofs[index].dataUrl, note: proofNote.trim() || undefined });
        setUploadingCount(capturedProofs.length - index - 1);
      }
      setCapturedProofs([]);
      setProofNote("");
