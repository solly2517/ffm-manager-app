...
export default function Home() {
  ...
  const addUser = trpc.admin.addUser.useMutation({ onSuccess: () => { setNewUserEmail(""); usersQuery.refetch(); } });
  const setRole = trpc.admin.setRole.useMutation({ onSuccess: () => usersQuery.refetch() });
  const removeUser = trpc.admin.removeUser.useMutation({ onSuccess: () => usersQuery.refetch() });
    ...
    <main className="manager-main">
      ...
      <section className="manager-content">
        {active === "dashboard" && <>
          ...
          <div className="stat-grid">{stats.map(({ label, value, detail, icon: Icon, tone }) => <Card key={label} className="stat-card"><CardContent><div className={`stat-icon ${tone}`}><Icon size={20}/></div><div className="stat-value">{value}</div><div className="stat-label">{label}</div><div className="stat-detail">{detail}</div></CardContent></Card>)}</div>
        ...
        {active === "admin" && isAdmin && <section className="blueprint-card section-card"><div className="section-heading"><div><p className="eyebrow">Access control</p><h1>Administration</h1><p className="muted">Manage FFM users, roles, and access to both apps.</p></div></div><div className="admin-callout"><ShieldCheck size={18}/><div><strong>Administrator account verified</strong><p>{user?.email || "dr.seleam@gmail.com"} is assigned the admin role automatically on first login.</p></div></div><div className="admin-add-row"><Input value={newUserEmail} onChange={(e) => setNewUserEmail(e.target.value)} placeholder="new.user@example.com" type="email"/><Button className="blueprint-button" disabled={!newUserEmail || addUser.isPending} onClick={() => addUser.mutate({ email: newUserEmail })}><UserPlus size={15}/> {addUser.isPending ? "Adding…" : "Add user"}</Button></div>{adminError && <div className="admin-feedback error">{adminError}</div>}{addUser.isSuccess && <div className="admin-feedback success">User added to the FFM directory.</div>}{usersQuery.isLoading ? <div className="admin-feedback">Loading users…</div> : usersQuery.data?.length === 0 ? <div className="admin-feedback">No users have been added yet.</div> : null}<div className="data-table"><div className="table-row table-head"><span>User</span><span>Role</span><span>Last sign-in</span><span>Status</span><span>Action</span></div>{(usersQuery.data || []).map((managedUser) => { const protectedUser = managedUser.email?.toLowerCase() === "dr.seleam@gmail.com" || managedUser.openId === user?.openId; return <div className="table-row" key={managedUser.id}><span className="person-cell"><span className="avatar">{initials(managedUser.name || managedUser.email || "U")}</span><strong>{managedUser.email || managedUser.name || "Pending user"}</strong></span><span><Badge className={managedUser.role === "admin" ? "badge-admin" : "badge-success"}>{managedUser.role === "admin" ? "Administrator" : "User"}</Badge></span><span>{managedUser.lastSignedIn ? new Date(managedUser.lastSignedIn).toLocaleDateString() : "Not signed in"}</span><span><Badge className="badge-success">Active</Badge></span><span className="admin-actions"><Button variant="ghost" size="sm" disabled={protectedUser || setRole.isPending} onClick={() => setRole.mutate({ id: managedUser.id, role: managedUser.role === "admin" ? "user" : "admin" })}>{managedUser.role === "admin" ? "Make user" : "Make admin"}</Button><Button variant="ghost" size="sm" disabled={protectedUser || removeUser.isPending} onClick={() => removeUser.mutate({ id: managedUser.id })}>Remove</Button></span></div>; })}</div></section>}
        {!["dashboard", "delegates", "tasks", "messages", "admin"].includes(active) && <section className="blueprint-card section-card empty-section"><div className="empty-illustration"><Map size={34}/></div><p className="eyebrow">FFM / {current.label.toUpperCase()}</p><h1>{current.label}</h1><p className="muted">This workspace is ready for your live data and operational records.</p><Button className="blueprint-button" onClick={() => setActive("dashboard")}>Return to dashboard</Button></section>}
      </section>
    </main>
  </div>;
}
