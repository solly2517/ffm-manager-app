...
export const appRouter = router({
  ...
  auth: router({
    ...
    // One-time setup: only works while the database has zero users, so it
    // can't be used to create additional admins later. Use invitations for that.
    bootstrapAdmin: publicProcedure
      .input(z.object({ email: z.string().trim().toLowerCase().email(), password: z.string().min(8).max(128), name: z.string().trim().min(2).max(120) }))
      .mutation(async ({ input, ctx }) => {
        const existingUserCount = await countUsers();
        if (existingUserCount > 0)
          throw new TRPCError({ code: "FORBIDDEN", message: "Setup has already been completed" });
        const passwordHash = await bcrypt.hash(input.password, 10);
        const user = await createLocalUser({ email: input.email, name: input.name, passwordHash, role: "admin" });
        if (!user)
          throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Unable to create admin account" });
        const sessionToken = await sdk.createSessionToken(user.openId, {
          expiresInMs: ONE_YEAR_MS,
          name: input.name,
        });
    ...
    changePassword: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user.passwordHash)
          throw new TRPCError({ code: "BAD_REQUEST", message: "No password set on this account yet" });
        const valid = await bcrypt.compare(input.currentPassword, ctx.user.passwordHash);
        if (!valid) throw new TRPCError({ code: "UNAUTHORIZED", message: "Current password is incorrect" });
        const passwordHash = await bcrypt.hash(input.newPassword, 10);
        await setUserPassword(ctx.user.id, passwordHash);
        return { success: true } as const;
      }),
  ...
  invitations: router({
    ...
    acceptMagic: publicProcedure
      .input(z.object({ token: z.string().min(20), password: z.string().min(8).max(128), name: z.string().trim().min(2).max(120) }))
      .mutation(async ({ input, ctx }) => {
        const invitation = await getInvitationByHash(tokenHash(input.token));
        if (
          !invitation ||
        ...
        const passwordHash = await bcrypt.hash(input.password, 10);
        await setUserPassword(invitedUser.id, passwordHash);
        await updateUserDisplayName(invitedUser.id, input.name);
        await acceptInvitation(invitation.id);
