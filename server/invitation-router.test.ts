...
describe("invitations.acceptMagic", () => {
  it("activates the invited role and issues a short-lived FFM session cookie", async () => {
    ...
    const result = await caller.invitations.acceptMagic({ token: "a".repeat(64), password: "correct-horse-battery", name: "M. Selim" });
  ...
  it("assigns an activated future Warehouse Hero to the verified Osama Ahmed Manager relationship", async () => {
    ...
    await expect(caller.invitations.acceptMagic({ token: "b".repeat(64), password: "correct-horse-battery", name: "Future Hero" })).resolves.toEqual({ success: true, role: "warehouse_hero" });
