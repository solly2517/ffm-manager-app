...
export async function getBackupArchive(userId: number, id: number) { const db = await getDb(); if (!db) return undefined; return (await db.select().from(backupArchives).where(and(eq(backupArchives.createdBy, userId), eq(backupArchives.id, id))).limit(1))[0]; }
...
const restoreKeys = ["users", "invitations", "clients", "doctors", "managerDelegateAssignments", "managerWarehouseHeroAssignments", "warehouseHeroLocations", "warehouseDeliveryProofs", "tasks", "visits", "evidence", "auditEvents", "messages", "userNotifications", "surgeries", "implantCatalogue", "surgeryImplants", "surgeryDeliveryProofs", "visitPlans", "geography"] as const;
export type FfmOperationalSnapshot = { format: "ffm-operational-snapshot-v1"; generatedAt: string; scope: { databaseRecords: boolean; evidenceManifest: boolean; originalBinaryFilesIncluded: boolean }; data: Record<(typeof restoreKeys)[number], unknown[]> };
export function validateFfmOperationalSnapshot(value: unknown): FfmOperationalSnapshot { if (!value || typeof value !== "object") throw new Error("Backup archive is not valid JSON"); const snapshot = value as Partial<FfmOperationalSnapshot>; if (snapshot.format !== "ffm-operational-snapshot-v1" || !snapshot.data || typeof snapshot.data !== "object") throw new Error("This file is not an FFM operational snapshot"); for (const key of restoreKeys) if (!Array.isArray(snapshot.data[key])) throw new Error(`Backup archive is missing ${key}`); return snapshot as FfmOperationalSnapshot; }
export function getSnapshotRecordCounts(snapshot: FfmOperationalSnapshot) { return Object.fromEntries(restoreKeys.map((key) => [key, snapshot.data[key].length])); }
export async function restoreFfmOperationalSnapshot(snapshot: FfmOperationalSnapshot) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const data = snapshot.data; const rows = <K extends (typeof restoreKeys)[number]>(key: K) => data[key] as any[]; await db.transaction(async (tx) => { for (const table of [userNotifications, surgeryDeliveryProofs, surgeryImplants, surgeries, evidence, visits, tasks, visitPlans, messages, warehouseDeliveryProofs, warehouseHeroLocations, managerDelegateAssignments, managerWarehouseHeroAssignments, doctors, geography, clients, invitations, implantCatalogue, auditEvents, users]) await tx.delete(table); const inserts: Array<[any, any[]]> = [[users, rows("users")], [invitations, rows("invitations")], [clients, rows("clients")], [doctors, rows("doctors")], [geography, rows("geography")], [managerDelegateAssignments, rows("managerDelegateAssignments")], [managerWarehouseHeroAssignments, rows("managerWarehouseHeroAssignments")], [warehouseHeroLocations, rows("warehouseHeroLocations")], [warehouseDeliveryProofs, rows("warehouseDeliveryProofs")], [tasks, rows("tasks")], [visits, rows("visits")], [evidence, rows("evidence")], [messages, rows("messages")], [userNotifications, rows("userNotifications")], [surgeries, rows("surgeries")], [implantCatalogue, rows("implantCatalogue")], [surgeryImplants, rows("surgeryImplants")], [surgeryDeliveryProofs, rows("surgeryDeliveryProofs")], [visitPlans, rows("visitPlans")], [auditEvents, rows("auditEvents")]]; for (const [table, values] of inserts) if (values.length) await tx.insert(table).values(values); }); return getSnapshotRecordCounts(snapshot); }
