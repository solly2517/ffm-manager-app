...
export async function listAllSurgeries() { const db = await getDb(); if (!db) return []; return db.select().from(surgeries).orderBy(surgeries.surgeryDate); }
...
export async function listSurgeryImplants(surgeryId: number) { const db = await getDb(); if (!db) return []; const [rows, catalogue] = await Promise.all([db.select().from(surgeryImplants).where(eq(surgeryImplants.surgeryId, surgeryId)).orderBy(desc(surgeryImplants.registeredAt)), listImplantCatalogue(true)]); const catalogueById = new Map(catalogue.map((item) => [item.id, item])); return rows.map((implant) => { const unitPrice = Number(implant.unitPrice ?? 0); const lineTotal = Number.isFinite(unitPrice) ? Number((unitPrice * implant.quantity).toFixed(2)) : 0; return { ...implant, unitPrice, currency: (implant.currency || "SAR").toUpperCase(), lineTotal, catalogueName: implant.implantCatalogueId ? catalogueById.get(implant.implantCatalogueId)?.name ?? null : null, catalogueManufacturer: implant.implantCatalogueId ? catalogueById.get(implant.implantCatalogueId)?.manufacturer ?? null : null, catalogueProductCode: implant.implantCatalogueId ? catalogueById.get(implant.implantCatalogueId)?.productCode ?? null : null }; }); }
