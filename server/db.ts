...
export async function listWarehouseDeliveryProofsForManager(managerId: number, includeAll = false) { const allowed = includeAll ? undefined : await listWarehouseHeroIdsForManager(managerId); if (!includeAll && !allowed?.length) return []; const db = await getDb(); if (!db) return []; const rows = await db.select({ id: warehouseDeliveryProofs.id, warehouseHeroId: warehouseDeliveryProofs.warehouseHeroId, warehouseHeroName: users.name, warehouseHeroEmail: users.email, note: warehouseDeliveryProofs.note, storageKey: warehouseDeliveryProofs.storageKey, mimeType: warehouseDeliveryProofs.mimeType, sizeBytes: warehouseDeliveryProofs.sizeBytes, capturedAt: warehouseDeliveryProofs.capturedAt }).from(warehouseDeliveryProofs).innerJoin(users, eq(warehouseDeliveryProofs.warehouseHeroId, users.id)).orderBy(desc(warehouseDeliveryProofs.capturedAt)); return rows.filter((row) => includeAll || allowed?.includes(row.warehouseHeroId)).map((row) => ({ ...row, url: `/manus-storage/${row.storageKey}` })); }
