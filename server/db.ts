...
import { InsertUser, users, invitations, clients, doctors, managerDelegateAssignments, managerWarehouseHeroAssignments, warehouseHeroLocations, warehouseDeliveryProofs, tasks, visits, evidence, auditEvents, messages, surgeries, surgeryImplants, surgeryDeliveryProofs, visitPlans, geography, clientErrorReports, weeklyBackupReminderSchedules } from "../drizzle/schema";
...
export async function listSurgeryImplants(surgeryId: number) { const db = await getDb(); if (!db) return []; return db.select().from(surgeryImplants).where(eq(surgeryImplants.surgeryId, surgeryId)).orderBy(desc(surgeryImplants.registeredAt)); }
