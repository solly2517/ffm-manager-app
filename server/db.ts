...
export async function exportFfmOperationalSnapshot() { const db = await getDb(); if (!db) throw new Error("Database is not available"); const [userRows, invitationRows, clientRows, doctorRows, delegateAssignments, warehouseAssignments, locationRows, warehouseProofRows, taskRows, visitRows, evidenceRows, auditRows, messageRows, notificationRows, surgeryRows, catalogueRows, implantRows, deliveryProofRows, visitPlanRows, geographyRows] = await Promise.all([db.select().from(users), db.select().from(invitations), db.select().from(clients), db.select().from(doctors), db.select().from(managerDelegateAssignments), db.select().from(managerWarehouseHeroAssignments), db.select().from(warehouseHeroLocations), db.select().from(warehouseDeliveryProofs), db.select().from(tasks), db.select().from(visits), db.select().from(evidence), db.select().from(auditEvents), db.select().from(messages), db.select().from(userNotifications), db.select().from(surgeries), db.select().from(implantCatalogue), db.select().from(surgeryImplants), db.select().from(surgeryDeliveryProofs), db.select().from(visitPlans), db.select().from(geography)]); return { format: "ffm-operational-snapshot-v1", generatedAt: new Date().toISOString(), scope: { databaseRecords: true, evidenceManifest: true, originalBinaryFilesIncluded: false }, data: { users: userRows, invitations: invitationRows, clients: clientRows, doctors: doctorRows, managerDelegateAssignments: delegateAssignments, managerWarehouseHeroAssignments: warehouseAssignments, warehouseHeroLocations: locationRows, warehouseDeliveryProofs: warehouseProofRows, tasks: taskRows, visits: visitRows, evidence: evidenceRows, auditEvents: auditRows, messages: messageRows, userNotifications: notificationRows, surgeries: surgeryRows, implantCatalogue: catalogueRows, surgeryImplants: implantRows, surgeryDeliveryProofs: deliveryProofRows, visitPlans: visitPlanRows, geography: geographyRows } }; }
