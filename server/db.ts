...
export async function listSurgeryImplants(surgeryId: number) { const db = await getDb(); if (!db) return []; const [rows, catalogue] = await Promise.all([db.select().from(surgeryImplants).where(eq(surgeryImplants.surgeryId, surgeryId)).orderBy(desc(surgeryImplants.registeredAt)), listImplantCatalogue(true)]); const catalogueById = new Map(catalogue.map((item) => [item.id, item])); return rows.map((implant) => ({ ...implant, catalogueName: implant.implantCatalogueId ? catalogueById.get(implant.implantCatalogueId)?.name ?? null : null, catalogueManufacturer: implant.implantCatalogueId ? catalogueById.get(implant.implantCatalogueId)?.manufacturer ?? null : null, catalogueProductCode: implant.implantCatalogueId ? catalogueById.get(implant.implantCatalogueId)?.productCode ?? null : null })); }
