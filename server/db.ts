...
import { InsertUser, users, departments, superManagerReportFilterPresets, monthlyDepartmentReportShares, invitations, clients, doctors, managerDelegateAssignments, managerSeniorities, topManagerManagerAssignments, managerDirections, managerWarehouseHeroAssignments, warehouseHeroLocations, warehouseDeliveryProofs, warehouseHandovers, tasks, visits, evidence, auditEvents, messages, userNotifications, surgeries, implantCatalogue, surgeryImplants, surgeryDeliveryProofs, visitPlans, weeklyVisitPlans, dailyActivityReports, geography, clientErrorReports, weeklyBackupReminderSchedules, googleDriveBackupConnections, backupArchives, travelExpenseClaims, travelExpenseLines } from "../drizzle/schema";
...
export type WeeklyWarehouseHandoverAnalytics = { weekStart: string; weekEnd: string; totalHandovers: number; acknowledgedHandovers: number; awaitingAcknowledgement: number; totalProofPhotos: number; rows: Array<{ id: number; completedAt: Date; warehouseHeroName: string; warehouseHeroEmail: string | null; recipientName: string; proofCount: number; acknowledgementStatus: "acknowledged" | "awaiting_acknowledgement"; acknowledgedAt: Date | null; acknowledgedByName: string | null; note: string | null }> };
export function warehouseHandoverAnalyticsCsv(analytics: WeeklyWarehouseHandoverAnalytics) { const cell = (value: string | number | Date | null | undefined) => { const raw = value instanceof Date ? value.toISOString() : String(value ?? ""); const safe = /^[=+\-@]/.test(raw) ? `'${raw}` : raw; return `"${safe.replace(/"/g, '""')}"`; }; const header = "handover_id,completed_at,warehouse_hero,warehouse_hero_email,recipient_name,proof_photo_count,acknowledgement_status,acknowledged_at,acknowledged_by,note"; const rows = analytics.rows.map(row => [row.id, row.completedAt, row.warehouseHeroName, row.warehouseHeroEmail, row.recipientName, row.proofCount, row.acknowledgementStatus, row.acknowledgedAt, row.acknowledgedByName, row.note].map(cell).join(",")); return `${header}\n${rows.join("\n")}${rows.length ? "\n" : ""}`; }
...
export async function getGoogleDriveBackupConnection(userId: number) { const db = await getDb(); if (!db) return undefined; return (await db.select().from(googleDriveBackupConnections).where(eq(googleDriveBackupConnections.userId, userId)).limit(1))[0]; }
export async function upsertGoogleDriveBackupConnection(input: typeof googleDriveBackupConnections.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database is not available"); await db.insert(googleDriveBackupConnections).values(input).onDuplicateKeyUpdate({ set: { googleEmail: input.googleEmail, folderId: input.folderId, encryptedRefreshToken: input.encryptedRefreshToken, connectedAt: new Date() } }); return getGoogleDriveBackupConnection(input.userId); }
export async function listBackupArchives(userId: number) { const db = await getDb(); if (!db) return []; return db.select().from(backupArchives).where(eq(backupArchives.createdBy, userId)).orderBy(desc(backupArchives.createdAt)).limit(20); }
export async function createBackupArchive(input: typeof backupArchives.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const result = await db.insert(backupArchives).values(input); return (await db.select().from(backupArchives).where(eq(backupArchives.id, Number(result[0].insertId))).limit(1))[0]; }
export async function completeBackupArchive(id: number, update: { googleDriveFileId: string; sizeBytes: number }) { const db = await getDb(); if (!db) throw new Error("Database is not available"); await db.update(backupArchives).set({ status: "completed", googleDriveFileId: update.googleDriveFileId, sizeBytes: update.sizeBytes, completedAt: new Date(), errorMessage: null }).where(eq(backupArchives.id, id)); }
export async function failBackupArchive(id: number, errorMessage: string) { const db = await getDb(); if (!db) throw new Error("Database is not available"); await db.update(backupArchives).set({ status: "failed", errorMessage: errorMessage.slice(0, 3000), completedAt: new Date() }).where(eq(backupArchives.id, id)); }
export async function exportFfmOperationalSnapshot() { const db = await getDb(); if (!db) throw new Error("Database is not available"); const [userRows, invitationRows, clientRows, doctorRows, delegateAssignments, warehouseAssignments, locationRows, warehouseProofRows, handoverRows, taskRows, visitRows, evidenceRows, auditRows, messageRows, notificationRows, surgeryRows, catalogueRows, implantRows, deliveryProofRows, visitPlanRows, geographyRows] = await Promise.all([db.select().from(users), db.select().from(invitations), db.select().from(clients), db.select().from(doctors), db.select().from(managerDelegateAssignments), db.select().from(managerWarehouseHeroAssignments), db.select().from(warehouseHeroLocations), db.select().from(warehouseDeliveryProofs), db.select().from(warehouseHandovers), db.select().from(tasks), db.select().from(visits), db.select().from(evidence), db.select().from(auditEvents), db.select().from(messages), db.select().from(userNotifications), db.select().from(surgeries), db.select().from(implantCatalogue), db.select().from(surgeryImplants), db.select().from(surgeryDeliveryProofs), db.select().from(visitPlans), db.select().from(geography)]); return { format: "ffm-operational-snapshot-v1", generatedAt: new Date().toISOString(), scope: { databaseRecords: true, evidenceManifest: true, originalBinaryFilesIncluded: false }, data: { users: userRows, invitations: invitationRows, clients: clientRows, doctors: doctorRows, managerDelegateAssignments: delegateAssignments, managerWarehouseHeroAssignments: warehouseAssignments, warehouseHeroLocations: locationRows, warehouseDeliveryProofs: warehouseProofRows, warehouseHandovers: handoverRows, tasks: taskRows, visits: visitRows, evidence: evidenceRows, auditEvents: auditRows, messages: messageRows, userNotifications: notificationRows, surgeries: surgeryRows, implantCatalogue: catalogueRows, surgeryImplants: implantRows, surgeryDeliveryProofs: deliveryProofRows, visitPlans: visitPlanRows, geography: geographyRows } }; }
...
/** Creates a brand-new local (email/password) user account. Throws if the email is already taken. */
export async function createLocalUser(input: { email: string; name: string; passwordHash: string; role: "user" | "manager" | "delegate" | "warehouse_hero" | "admin" }) {
  const db = await getDb(); if (!db) throw new Error("Database is not available");
  const email = input.email.toLowerCase();
  const existing = await getUserByEmail(email);
  if (existing) throw new Error("An account with this email already exists");
  const openId = `local:${email}`;
  await db.insert(users).values({ openId, email, name: input.name, passwordHash: input.passwordHash, loginMethod: "password", role: input.role, lastSignedIn: new Date() });
...
export async function listManagerSenioritiesForAdmin() { const db = await getDb(); if (!db) return []; const [managers, seniorities, scopes] = await Promise.all([listManagers(), db.select().from(managerSeniorities), db.select().from(topManagerManagerAssignments)]); const byManager = new Map(seniorities.map(row => [row.managerId, row])); return managers.map(manager => ({ managerId: manager.id, name: manager.name || manager.email || "Manager", email: manager.email || null, level: byManager.get(manager.id)?.level ?? "manager", setAt: byManager.get(manager.id)?.setAt ?? null, directedManagerCount: scopes.filter(scope => scope.topManagerId === manager.id).length })); }
export async function getManagerSeniority(managerId: number) { const db = await getDb(); if (!db) return undefined; return (await db.select().from(managerSeniorities).where(eq(managerSeniorities.managerId, managerId)).limit(1))[0]; }
export async function isTopManager(managerId: number) { return (await getManagerSeniority(managerId))?.level === "top_manager"; }
export async function setManagerSeniority(input: typeof managerSeniorities.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database is not available"); await db.insert(managerSeniorities).values(input).onDuplicateKeyUpdate({ set: { level: input.level, setBy: input.setBy, setAt: new Date() } }); return getManagerSeniority(input.managerId); }
export async function listManagerDirectionScopes() { const db = await getDb(); if (!db) return []; const [scopes, people] = await Promise.all([db.select().from(topManagerManagerAssignments).orderBy(topManagerManagerAssignments.createdAt), db.select().from(users)]); const byId = new Map(people.map(person => [person.id, person])); return scopes.map(scope => ({ ...scope, topManagerName: byId.get(scope.topManagerId)?.name || byId.get(scope.topManagerId)?.email || "Top Manager", topManagerEmail: byId.get(scope.topManagerId)?.email || null, managerName: byId.get(scope.managerId)?.name || byId.get(scope.managerId)?.email || "Manager", managerEmail: byId.get(scope.managerId)?.email || null })); }
export async function createTopManagerManagerAssignment(input: typeof topManagerManagerAssignments.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const result = await db.insert(topManagerManagerAssignments).values(input); return (await db.select().from(topManagerManagerAssignments).where(eq(topManagerManagerAssignments.id, Number(result[0].insertId))).limit(1))[0]; }
export async function removeTopManagerManagerAssignment(id: number) { const db = await getDb(); if (!db) throw new Error("Database is not available"); await db.delete(topManagerManagerAssignments).where(eq(topManagerManagerAssignments.id, id)); return { success: true as const }; }
export async function isManagerDirectedByTopManager(topManagerId: number, managerId: number) { const db = await getDb(); if (!db) return false; const rows = await db.select({ id: topManagerManagerAssignments.id }).from(topManagerManagerAssignments).where(and(eq(topManagerManagerAssignments.topManagerId, topManagerId), eq(topManagerManagerAssignments.managerId, managerId))).limit(1); return rows.length > 0; }
export async function listManagersForTopManager(topManagerId: number) { const db = await getDb(); if (!db) return []; const scopes = await db.select({ managerId: topManagerManagerAssignments.managerId }).from(topManagerManagerAssignments).where(eq(topManagerManagerAssignments.topManagerId, topManagerId)); if (!scopes.length) return []; const ids = new Set(scopes.map(scope => scope.managerId)); return (await listManagers()).filter(manager => ids.has(manager.id)); }
export async function createManagerDirection(input: typeof managerDirections.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const result = await db.insert(managerDirections).values(input); return (await db.select().from(managerDirections).where(eq(managerDirections.id, Number(result[0].insertId))).limit(1))[0]; }
export async function listManagerDirectionsForTopManager(topManagerId: number) { const db = await getDb(); if (!db) return []; const [directions, people] = await Promise.all([db.select().from(managerDirections).where(eq(managerDirections.topManagerId, topManagerId)).orderBy(desc(managerDirections.createdAt)), db.select().from(users)]); const byId = new Map(people.map(person => [person.id, person])); return directions.map(direction => ({ ...direction, managerName: byId.get(direction.managerId)?.name || byId.get(direction.managerId)?.email || "Manager", managerEmail: byId.get(direction.managerId)?.email || null })); }
export async function listManagerDirectionsForManager(managerId: number) { const db = await getDb(); if (!db) return []; const [directions, people] = await Promise.all([db.select().from(managerDirections).where(eq(managerDirections.managerId, managerId)).orderBy(desc(managerDirections.createdAt)), db.select().from(users)]); const byId = new Map(people.map(person => [person.id, person])); return directions.map(direction => ({ ...direction, topManagerName: byId.get(direction.topManagerId)?.name || byId.get(direction.topManagerId)?.email || "Manager", topManagerEmail: byId.get(direction.topManagerId)?.email || null })); }
export async function completeManagerDirection(id: number, managerId: number) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const direction = (await db.select().from(managerDirections).where(and(eq(managerDirections.id, id), eq(managerDirections.managerId, managerId))).limit(1))[0]; if (!direction) return undefined; await db.update(managerDirections).set({ status: "completed", completedAt: new Date() }).where(eq(managerDirections.id, id)); return (await db.select().from(managerDirections).where(eq(managerDirections.id, id)).limit(1))[0]; }
...
const matchesInclusiveDateRange = (value: Date | string, input: { from?: string; to?: string }) => {
  const timestamp = new Date(value).getTime();
  const from = input.from ? new Date(`${input.from}T00:00:00.000Z`).getTime() : Number.NEGATIVE_INFINITY;
  const to = input.to ? new Date(`${input.to}T23:59:59.999Z`).getTime() : Number.POSITIVE_INFINITY;
  return timestamp >= from && timestamp <= to;
};
export async function getDepartmentDashboardTotals(input: { from?: string; to?: string } = {}) {
  ...
  return departmentRows.map(department => {
    ...
    const taskRowsForDepartment = taskRows.filter(task => memberDepartment.get(task.delegateId) === normalized && matchesInclusiveDateRange(task.scheduledAt, input));
    const weeklyPlanCount = weeklyRows.filter(plan => memberDepartment.get(plan.authorId) === normalized && matchesInclusiveDateRange(plan.weekOf, input)).length;
    const dailyReportCount = dailyRows.filter(report => memberDepartment.get(report.authorId) === normalized && matchesInclusiveDateRange(report.reportDate, input)).length;
...
export async function listDepartmentAuditEvents(input: { from?: string; to?: string; limit?: number } = {}) { const db = await getDb(); if (!db) return []; const [events, people] = await Promise.all([db.select().from(auditEvents).orderBy(desc(auditEvents.createdAt)).limit(input.limit ?? 500), db.select().from(users)]); const byId = new Map(people.map(person => [person.id, person])); return events.filter(event => event.action.startsWith("department.") && matchesInclusiveDateRange(event.createdAt, input)).map(event => ({ ...event, actorName: byId.get(event.actorId)?.name || byId.get(event.actorId)?.email || "FFM Administrator", actorEmail: byId.get(event.actorId)?.email || null })); }
export async function listSuperManagerReportFilterPresets(userId: number, includeShared = true) { const db = await getDb(); if (!db) return []; const where = includeShared ? or(eq(superManagerReportFilterPresets.userId, userId), eq(superManagerReportFilterPresets.isShared, true)) : eq(superManagerReportFilterPresets.userId, userId); return db.select().from(superManagerReportFilterPresets).where(where).orderBy(superManagerReportFilterPresets.name); }
...
export async function removeSuperManagerReportFilterPreset(id: number, userId: number, allowShared = false) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const preset = (await db.select().from(superManagerReportFilterPresets).where(eq(superManagerReportFilterPresets.id, id)).limit(1))[0]; if (!preset || (preset.userId !== userId && !(allowShared && preset.isShared))) return { deleted: 0 }; const result = await db.delete(superManagerReportFilterPresets).where(eq(superManagerReportFilterPresets.id, id)); return { deleted: Number(result[0]?.affectedRows ?? 0) }; }
export async function createMonthlyDepartmentReportShare(input: typeof monthlyDepartmentReportShares.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const result = await db.insert(monthlyDepartmentReportShares).values(input); return (await db.select().from(monthlyDepartmentReportShares).where(eq(monthlyDepartmentReportShares.id, Number(result[0].insertId))).limit(1))[0]; }
export async function getActiveMonthlyDepartmentReportShare(tokenHash: string) { const db = await getDb(); if (!db) return undefined; const share = (await db.select().from(monthlyDepartmentReportShares).where(eq(monthlyDepartmentReportShares.tokenHash, tokenHash)).limit(1))[0]; return share && share.expiresAt.getTime() > Date.now() ? share : undefined; }
export async function listMonthlyDepartmentReportShares() { const db = await getDb(); if (!db) return []; const [shares, people] = await Promise.all([db.select().from(monthlyDepartmentReportShares).orderBy(desc(monthlyDepartmentReportShares.createdAt)), db.select().from(users)]); const byId = new Map(people.map(person => [person.id, person])); return shares.map(share => ({ id: share.id, month: share.month, createdAt: share.createdAt, expiresAt: share.expiresAt, createdByName: byId.get(share.createdBy)?.name || byId.get(share.createdBy)?.email || "Administrator", createdByEmail: byId.get(share.createdBy)?.email || null, active: share.expiresAt.getTime() > Date.now() })); }
export async function revokeMonthlyDepartmentReportShare(id: number) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const result = await db.delete(monthlyDepartmentReportShares).where(eq(monthlyDepartmentReportShares.id, id)); return { deleted: Number(result[0]?.affectedRows ?? 0) }; }
export async function updateMonthlyDepartmentReportShareExpiry(id: number, expiresAt: Date) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const share = (await db.select().from(monthlyDepartmentReportShares).where(eq(monthlyDepartmentReportShares.id, id)).limit(1))[0]; if (!share || share.expiresAt.getTime() <= Date.now()) return undefined; await db.update(monthlyDepartmentReportShares).set({ expiresAt }).where(eq(monthlyDepartmentReportShares.id, id)); return (await db.select().from(monthlyDepartmentReportShares).where(eq(monthlyDepartmentReportShares.id, id)).limit(1))[0]; }
export async function getWarehouseHeroLeadActivity(managerId: number) { const db = await getDb(); if (!db) return []; const [heroes, taskRows, proofRows, locationRows] = await Promise.all([listWarehouseHeroesForManager(managerId), db.select().from(tasks), db.select().from(warehouseDeliveryProofs), db.select().from(warehouseHeroLocations)]); const today = new Date(); const todayStart = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate())); const recentStart = new Date(todayStart.getTime() - 13 * 24 * 60 * 60 * 1000); return heroes.map(hero => { const heroTasks = taskRows.filter(task => task.delegateId === hero.id); const todayTasks = heroTasks.filter(task => task.scheduledAt >= todayStart && task.scheduledAt < new Date(todayStart.getTime() + 24 * 60 * 60 * 1000)); const overdueProofTasks = heroTasks.filter(task => task.scheduledAt < todayStart && task.status !== "cancelled" && !proofRows.some(proof => proof.warehouseHeroId === hero.id && proof.capturedAt >= task.scheduledAt)); const recentProofs = proofRows.filter(proof => proof.warehouseHeroId === hero.id && proof.capturedAt >= recentStart).sort((a, b) => b.capturedAt.getTime() - a.capturedAt.getTime()).slice(0, 4); const location = locationRows.find(item => item.warehouseHeroId === hero.id); return { id: hero.id, name: hero.name || hero.email || "Warehouse Hero", email: hero.email || null, todayTaskCount: todayTasks.length, openTaskCount: heroTasks.filter(task => task.status === "pending" || task.status === "in_progress").length, completedTaskCount: heroTasks.filter(task => task.status === "completed").length, overdueProofTaskCount: overdueProofTasks.length, overdueProofTasks: overdueProofTasks.slice(0, 5).map(task => ({ id: task.id, scheduledAt: task.scheduledAt, status: task.status })), recentProofCount: recentProofs.length, recentProofs: recentProofs.map(proof => ({ id: proof.id, note: proof.note, capturedAt: proof.capturedAt, mimeType: proof.mimeType, captureSource: proof.captureSource })), latestLocationAt: location?.capturedAt || null }; }); }
export function warehouseHeroLeadActivityCsv(rows: Awaited<ReturnType<typeof getWarehouseHeroLeadActivity>>) { const cell = (value: string | number | null | undefined) => { const raw = String(value ?? ""); const safe = /^[=+\-@]/.test(raw) ? `'${raw}` : raw; return `"${safe.replace(/"/g, '""')}"`; }; const header = "warehouse_hero,email,today_tasks,open_tasks,completed_tasks,overdue_proof_tasks,recent_proofs,latest_gps_activity"; const lines = rows.map(row => [row.name, row.email, row.todayTaskCount, row.openTaskCount, row.completedTaskCount, row.overdueProofTaskCount, row.recentProofCount, row.latestLocationAt?.toISOString()].map(cell).join(",")); return `${header}\n${lines.join("\n")}${lines.length ? "\n" : ""}`; }
...
function enrichWorkLogRows<T extends { authorId: number; delegateId: number | null; reviewedBy: number | null }>(rows: T[], people: Array<{ id: number; name: string | null; email: string | null }>) { const byId = new Map(people.map((person) => [person.id, person])); return rows.map((row) => { const author = byId.get(row.authorId); const delegate = row.delegateId ? byId.get(row.delegateId) : undefined; const reviewer = row.reviewedBy ? byId.get(row.reviewedBy) : undefined; return { ...row, authorName: author?.name || author?.email || "FFM member", authorEmail: author?.email || null, delegateName: delegate?.name || delegate?.email || null, delegateEmail: delegate?.email || null, reviewerName: reviewer?.name || reviewer?.email || null, reviewerEmail: reviewer?.email || null }; }); }
export async function listAllWeeklyVisitPlans() { const db = await getDb(); if (!db) return []; const [rows, people] = await Promise.all([db.select().from(weeklyVisitPlans).orderBy(desc(weeklyVisitPlans.weekOf)), db.select().from(users)]); return enrichWorkLogRows(rows, people); }
export async function listWeeklyVisitPlansForAuthor(authorId: number) { return (await listAllWeeklyVisitPlans()).filter((row) => row.authorId === authorId); }
export async function listWeeklyVisitPlansForDelegate(delegateId: number) { return listWeeklyVisitPlansForAuthor(delegateId); }
export async function listWeeklyVisitPlansForManager(managerId: number) { const allowed = await listDelegateIdsForManager(managerId); return (await listAllWeeklyVisitPlans()).filter((row) => row.authorId === managerId || (row.delegateId != null && allowed.includes(row.delegateId))); }
...
export async function listAllDailyActivityReports() { const db = await getDb(); if (!db) return []; const [rows, people] = await Promise.all([db.select().from(dailyActivityReports).orderBy(desc(dailyActivityReports.reportDate), desc(dailyActivityReports.createdAt)), db.select().from(users)]); return enrichWorkLogRows(rows, people); }
export async function listDailyActivityReportsForAuthor(authorId: number) { return (await listAllDailyActivityReports()).filter((row) => row.authorId === authorId); }
export async function listDailyActivityReportsForDelegate(delegateId: number) { return listDailyActivityReportsForAuthor(delegateId); }
export async function listDailyActivityReportsForManager(managerId: number) { const allowed = await listDelegateIdsForManager(managerId); return (await listAllDailyActivityReports()).filter((row) => row.authorId === managerId || (row.delegateId != null && allowed.includes(row.delegateId))); }
...
export async function createWarehouseHandover(input: typeof warehouseHandovers.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const result = await db.insert(warehouseHandovers).values(input); return (await db.select().from(warehouseHandovers).where(eq(warehouseHandovers.id, Number(result[0].insertId))).limit(1))[0]; }
export async function listWarehouseHandovers() { const db = await getDb(); if (!db) return []; const [handovers, people, proofs] = await Promise.all([db.select().from(warehouseHandovers).orderBy(desc(warehouseHandovers.createdAt)), db.select().from(users), db.select().from(warehouseDeliveryProofs)]); const byId = new Map(people.map(person => [person.id, person])); return handovers.map(handover => { const hero = byId.get(handover.warehouseHeroId); const acknowledgedBy = handover.acknowledgedBy ? byId.get(handover.acknowledgedBy) : undefined; const handoverProofs = proofs.filter(proof => proof.handoverId === handover.id); return { ...handover, warehouseHeroName: hero?.name || hero?.email || "Warehouse Hero", warehouseHeroEmail: hero?.email || null, acknowledgedByName: acknowledgedBy?.name || acknowledgedBy?.email || null, signatureUrl: `/manus-storage/${handover.signatureStorageKey}`, proofCount: handoverProofs.length, proofs: handoverProofs.map(proof => ({ id: proof.id, url: `/manus-storage/${proof.storageKey}`, capturedAt: proof.capturedAt, note: proof.note })) }; }); }
export async function acknowledgeWarehouseHandover(id: number, acknowledgedBy: number) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const handover = (await db.select().from(warehouseHandovers).where(eq(warehouseHandovers.id, id)).limit(1))[0]; if (!handover || handover.acknowledgedAt) return undefined; const acknowledgedAt = new Date(); await db.update(warehouseHandovers).set({ acknowledgedBy, acknowledgedAt }).where(eq(warehouseHandovers.id, id)); return (await db.select().from(warehouseHandovers).where(eq(warehouseHandovers.id, id)).limit(1))[0]; }
export async function getWeeklyWarehouseHandoverAnalytics(weekStart?: string): Promise<WeeklyWarehouseHandoverAnalytics> { const start = weekStart ? new Date(`${weekStart}T00:00:00.000Z`) : new Date(); if (Number.isNaN(start.getTime())) throw new Error("A valid ISO week start is required"); if (!weekStart) { const offset = (start.getUTCDay() + 6) % 7; start.setUTCDate(start.getUTCDate() - offset); start.setUTCHours(0, 0, 0, 0); } const end = new Date(start); end.setUTCDate(end.getUTCDate() + 7); const empty = { weekStart: start.toISOString().slice(0, 10), weekEnd: end.toISOString().slice(0, 10), totalHandovers: 0, acknowledgedHandovers: 0, awaitingAcknowledgement: 0, totalProofPhotos: 0, rows: [] }; const db = await getDb(); if (!db) return empty; const [handovers, people, proofs] = await Promise.all([db.select().from(warehouseHandovers), db.select().from(users), db.select().from(warehouseDeliveryProofs)]); const peopleById = new Map(people.map(person => [person.id, person])); const rows = handovers.filter(handover => handover.createdAt >= start && handover.createdAt < end).sort((left, right) => right.createdAt.getTime() - left.createdAt.getTime()).map(handover => { const hero = peopleById.get(handover.warehouseHeroId); const acknowledgedBy = handover.acknowledgedBy ? peopleById.get(handover.acknowledgedBy) : null; return { id: handover.id, completedAt: handover.createdAt, warehouseHeroName: hero?.name || hero?.email || "Warehouse Hero", warehouseHeroEmail: hero?.email || null, recipientName: handover.recipientName, proofCount: proofs.filter(proof => proof.handoverId === handover.id).length, acknowledgementStatus: handover.acknowledgedAt ? "acknowledged" as const : "awaiting_acknowledgement" as const, acknowledgedAt: handover.acknowledgedAt, acknowledgedByName: acknowledgedBy?.name || acknowledgedBy?.email || null, note: handover.note }; }); const acknowledgedHandovers = rows.filter(row => row.acknowledgementStatus === "acknowledged").length; return { ...empty, totalHandovers: rows.length, acknowledgedHandovers, awaitingAcknowledgement: rows.length - acknowledgedHandovers, totalProofPhotos: rows.reduce((sum, row) => sum + row.proofCount, 0), rows }; }
...
export async function listSurgeriesForManager(managerId: number) { const allowed = await listDelegateIdsForManager(managerId); return (await listAllSurgeries()).filter((surgery) => surgery.assignedManagerId === managerId || (surgery.delegateId != null && allowed.includes(surgery.delegateId))); }
export async function listSurgeriesForTopManager(topManagerId: number) { const managedManagers = await listManagersForTopManager(topManagerId); const managerIds = new Set(managedManagers.map(manager => manager.id)); if (!managerIds.size) return []; const delegateIds = new Set<number>(); const managerDelegateRows = await Promise.all(managedManagers.map(manager => listDelegateIdsForManager(manager.id))); managerDelegateRows.forEach(ids => ids.forEach(delegateId => delegateIds.add(delegateId))); return (await listAllSurgeries()).filter((surgery) => (surgery.assignedManagerId != null && managerIds.has(surgery.assignedManagerId)) || (surgery.delegateId != null && delegateIds.has(surgery.delegateId))); }
export async function listAllSurgeries() { const db = await getDb(); if (!db) return []; const [records, people] = await Promise.all([db.select().from(surgeries).orderBy(surgeries.surgeryDate), db.select().from(users)]); const peopleById = new Map(people.map((person) => [person.id, person])); return records.map((surgery) => { const delegate = surgery.delegateId ? peopleById.get(surgery.delegateId) : undefined; const manager = surgery.assignedManagerId ? peopleById.get(surgery.assignedManagerId) : undefined; return { ...surgery, delegateName: delegate?.name || delegate?.email || "No Delegate assigned", delegateEmail: delegate?.email || null, managerName: manager?.name || manager?.email || null, managerEmail: manager?.email || null }; }); }
...
export async function getDetailedSurgeryReport(filters?: { from?: string; to?: string; delegateIds?: number[] }) {
  ...
  return surgeryRows
    .filter((surgery) => (!filters?.delegateIds || (surgery.delegateId != null && filters.delegateIds.includes(surgery.delegateId))) && (!from || surgery.surgeryDate >= from) && (!to || surgery.surgeryDate <= to))
    .map((surgery) => {
      const client = clientsById.get(surgery.clientId);
      const delegate = surgery.delegateId != null ? peopleById.get(surgery.delegateId) : undefined;
      const manager = surgery.assignedManagerId != null ? peopleById.get(surgery.assignedManagerId) : surgery.delegateId != null ? managerByDelegateId.get(surgery.delegateId) : undefined;
