...
export async function searchImplantCatalogue(query = "", limit = 100) { const db = await getDb(); if (!db) return []; const safeLimit = Math.min(Math.max(limit, 1), 200); const term = query.trim(); if (!term) return db.select().from(implantCatalogue).orderBy(implantCatalogue.name).limit(safeLimit); const pattern = `%${term}%`; return db.select().from(implantCatalogue).where(or(like(implantCatalogue.name, pattern), like(implantCatalogue.productCode, pattern), like(implantCatalogue.manufacturer, pattern))).orderBy(implantCatalogue.name).limit(safeLimit); }
